...
int ToProtoData(kortex_driver::DeviceHandle input, DeviceHandle *output)
{
...
int ToProtoData(kortex_driver::Empty input, Empty *output)
{

	return 0;
}
int ToProtoData(kortex_driver::NotificationOptions input, NotificationOptions *output)
{
...
int ToProtoData(kortex_driver::SafetyHandle input, SafetyHandle *output)
{
	output->set_identifier(input.identifier);

	return 0;
}
int ToProtoData(kortex_driver::NotificationHandle input, NotificationHandle *output)
{
	output->set_identifier(input.identifier);

	return 0;
}
int ToProtoData(kortex_driver::SafetyNotification input, SafetyNotification *output)
{
...
int ToProtoData(kortex_driver::Timestamp input, Timestamp *output)
{
...
int ToProtoData(kortex_driver::UserProfileHandle input, UserProfileHandle *output)
{
	output->set_identifier(input.identifier);
...
int ToProtoData(kortex_driver::Connection input, Connection *output)
{
	ToProtoData(input.user_handle, output->mutable_user_handle());
